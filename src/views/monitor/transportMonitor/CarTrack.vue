...
import { getCarSnPointInfo } from '@/api/device_new'
...
export default {
  data() {
    return {
      ...
      isPosition: false,
      isSuccessed: true, // 查询轨迹时失败后判断为了不让重复多次查询
      monitorInfoBox: null,
      trackInfoBox: null,
      point: null,
      type: null,
  ...
  mounted() {
    this.serviceId = this.carInfo.sid
    carDataInfo({
    ...
    if (this.serviceId === 213405) {
      getCarSnPointInfo({
        sn: this.carInfo.cid,
        start: this._dateFormat(new Date(this.stime), 'YYYY-MM-DD HH:mm'),
        end: this._dateFormat(new Date(this.etime), 'YYYY-MM-DD HH:mm')
      })
        .then(res => {
          if (res.data.Code === 200) {
  ...
  methods: {
    ...
    setTrackInfoBox(point, type) {
      if (this.serviceId === 213405) {
        let itemPushArr = ''
        const time = new Date(point.create_time).getTime()
        ...
        this.trackInfoBox = new BMapLib.InfoBox(this.map, itemPushArr, {
          boxStyle: 'transform: translateY(-20px)',
          boxClass: type === 'currentPosition' ? 'carInfoBox2' : 'carInfoBox',
        ...
        if (this.sensorMap.length) {
          this.sensorMap.forEach(item => {
            temp += `<div class="carInfoItem"><div class="infoItemTitle">${
              item.name
            }</div><div class="infoItemContent">${point[
              'sensor' + item.sensor
            ] + item.unit}</div></div>`
          })
        }
        let itemPushArr = `<div class="carInfoWindow"><div class="carInfoHeader0"><abbr>${
          this.$route.query.carNumber
        }</abbr></div><div class="carInfoContent">
        ${temp}
        <div class="carInfoItem"><div class="infoItemTitle">电量</div><div class="infoItemContent">${point.power +
          '%'}</div></div>
        <div class="carInfoItem"><div class="infoItemTitle">${timeText}</div><div class="infoItemContent">${
          point.create_time
        }</div></div>
