<template>
  <div>
    <div id="carmap_container" class="carmap_container">
      ...
      <!-- <el-button size="small" type="primary" style="position:absolute;left:180px;top:20px;" icon="el-icon-location" @click="checkPosition">
        {{ isPosition?'关闭实时位置':'查看实时位置' }}
      </el-button> -->
    </div>
  </div>
</template>
...
import { getCarPointInfo } from '@/api/device_new'
...
export default {
  data() {
    return {
      ...
      map: null,
      maploading: true,
      ak: 'kmGLT5EjKCWL5D5o2Ss9godDaA2GLZY9',
      entity_name: '',
      start: '',
      ...
      initlongitude: '', // 为了保存上次精度值
      initlatitude: '', // 为了保存上次精度值
      points: [],
      isPosition: false,
      endPlace: '',
      startPlace: '',
  ...
  mounted() {
    ...
    this.initHeigh = mapHeight
    const map = document.getElementById('carmap_container')
    map.style.cssText = `height:${mapHeight}px`
    this.entity_name = this.$route.query.carSn
    this.serviceId = parseInt(this.$route.query.carServiceID)
    this.start = parseInt(new Date(this.$route.query.start).getTime() / 1000)
    if (this.$route.query.end) {
      this.end = parseInt(new Date(this.$route.query.end).getTime() / 1000)
    } else {
    ...
    this.map.centerAndZoom(new BMap.Point(121.47, 31.23), 11)
    this.map.enableScrollWheelZoom(true)
    window.map = this.map
    // eslint-disable-next-line no-undef
    const opt1 = { anchor: BMAP_ANCHOR_TOP_RIGHT }
    ...
    window.addEventListener('resize', () => {
      if (this.$route.path !== '/monitor/trackReplay') {
        return
      }
      const mapHeight = window.innerHeight - 141
      const map = document.getElementById('carmap_container')
    ...
    if (this.serviceId === 213405) {
      getCarPointInfo({
        carId: this.$route.query.carId,
        start: this._dateFormat(
          new Date(this.start * 1000),
          'YYYY-MM-DD HH:mm'
        ),
        end: this._dateFormat(new Date(this.end * 1000), 'YYYY-MM-DD HH:mm')
      })
        .then(res => {
          if (res.data.Code === 200) {
            this.pointHistoryDataList = res.data.Data
          } else {
            this.pointHistoryDataList = []
          }
        })
        .catch(err => {
          console.error(err)
    ...
    async function loadpoints() {
      const interval = 3600 * 24
      for (
        var i = 0;
        i <= Math.ceil((that.end - that.start) / interval) - 1;
        i++
      ) {
        let start = that.start + i * interval
        let end = that.start + (i + 1) * interval
        await that.getTrackPoint(start, end).then(res => {
    ...
    loadpoints()
      ...
      .catch(err => {
        console.error(err)
        this.$message.error('车辆轨迹点加载错误')
      })
      .finally(() => {
        this.maploading = false
      })

    if (this.serviceId === 164138) {
      carDataInfo({
        carId: this.$route.query.carId
      })
        .then(res => {
          if (res.data.Code === 200) {
            this.sensorMap = res.data.Data
          } else {
            this.sensorMap = []
          }
        })
  ...
  methods: {
    getTrackPoint(start, end) {
      return new Promise((resolve, reject) => {
        axios
          .get(BASE_URI + '/api/Forward/ForwardBaidu', {
            params: {
        ...
        if (totalPoints.length !== 0) {
          ...
          for (let i = 0, len = totalPoints.length; i < len - 1; i++) {
            ...
            if (
              ...
              //     // partImgStart.src = 'https://cdn.sinocold.net/images/monitorDashboard/start2.png'
              //     let next = nextPixel;
              ...
              //     // partImgEnd.src = 'https://cdn.sinocold.net/images/monitorDashboard/end2.png'
    ...
    setTrackInfoBox(mapPonint, type) {
      // 164138
      let itemPushArr = ''
      if (this.serviceId === 213405) {
        const time = new Date(mapPonint.create_time).getTime()

        let points = []
        this.pointHistoryDataList.forEach(point => {
          const temp = point.pointHistoryDataList.find(item => {
            const updateTime = new Date(item.UPDATE_TIME).getTime()
            return updateTime > time - 60000 && updateTime < time + 60000
          })
          if (temp) {
            points.push(temp)
          } else {
            points.push({
              NAME: point.pointHistoryDataList[0].NAME,
              VALUE: '',
              unit: ''
            })
          }
        ...
        points.forEach(item => {
          temp += `<div class="carInfoItem"><div class="infoItemTitle">${
            item.NAME
          }</div><div class="infoItemContent">${item.VALUE +
            item.unit}</div></div>`
        })
        itemPushArr = `<div class="carInfoWindow"><div class="carInfoHeader0"><abbr>${
          this.$route.query.carNumber
        }</abbr></div><div class="carInfoContent">
        ${points.length ? temp : ''}
        <div class="carInfoItem"><div class="infoItemTitle">${timeText}</div><div class="infoItemContent">${
          mapPonint.create_time
        }</div>
        ${!points.length ? temp : ''}
        </div>
        </div></div>`
      } else if (this.serviceId === 164138) {
        const timeText = type === 'currentPosition' ? '更新时间' : '定位时间'
        let temp = ''
        if (this.sensorMap.length) {
          this.sensorMap.forEach(item => {
            temp += `<div class="carInfoItem"><div class="infoItemTitle">${
              item.name
            }</div><div class="infoItemContent">${mapPonint[
              'sensor' + item.sensor
            ] + item.unit}</div></div>`
          })
        }
        ...
        ${temp}
        <div class="carInfoItem"><div class="infoItemTitle">电量</div><div class="infoItemContent">${mapPonint.power +
          '%'}</div></div>
        <div class="carInfoItem"><div class="infoItemTitle">${timeText}</div><div class="infoItemContent">${
          mapPonint.create_time
        }</div></div>
        ...
        <div class="carInfoWindow">
          <div class="carInfoContent">
            ...
            <div class="carInfoItem">
              <div class="infoItemTitle">温度</div>
              <div class="infoItemContent">
                ${ mapPonint.tempture }
              </div>
            </div>
            ${mapPonint.humidity > 0 ?
            `
              <div class="carInfoItem">
                <div class="infoItemTitle">湿度</div>
                <div class="infoItemContent">
                  ${ mapPonint.humidity }
                </div>
              </div>
      ...
      this.trackInfoBox = new BMapLib.InfoBox(this.map, itemPushArr, {
        boxClass: type === 'currentPosition' ? 'carInfoBox2' : this.serviceId === 159810 ? 'carInfoBox3' : 'carInfoBox',
        closeIconMargin: '36px 20px 0 0',
        alignBottom: false,
        closeIconUrl: 'https://cdn.sinocold.net/images/baiduMap/closeinfowindow.png'
      })
      this.trackInfoBox.open(mapPonint)
      this.map.panTo(mapPonint)
    }
  }
}
</script>
