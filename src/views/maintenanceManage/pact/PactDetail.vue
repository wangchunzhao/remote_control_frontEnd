<template>
  ...
  >
        ...
        <el-col :span="8">
          <div class="column-value">
            <span v-if="!item.editable">{{
              _dateFormat(item.maintainFirst, 'YYYY-MM-DD')
            }}</span>
            <el-date-picker
              v-else
              type="date"
            ...
            ></el-date-picker>
            <el-button
              v-if="companyId === data.partyB && data.contractStatus !== 2"
              type="text"
              class="edit-btn"
    ...
    <span
      v-if="companyId === data.partyB && data.contractStatus === 0"
      slot="footer"
      class="dialog-footer"
    >
